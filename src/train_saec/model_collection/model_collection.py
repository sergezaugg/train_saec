...
class Encoder_reshape(nn.Module):
    def __init__(self, n_ch_in = 3, n_ch_convout = 32):
        super().__init__()
        self.conv0 = nn.Sequential(
            nn.Conv2d(n_ch_in, 32, kernel_size=3, padding=1),   
            nn.BatchNorm2d(32),
        ...
        self.conv3 = nn.Sequential(
            ...
            nn.MaxPool2d(4) )
        self.conv4 = nn.Sequential(
            nn.Conv2d(128, n_ch_convout, kernel_size=3, padding=1),
            nn.BatchNorm2d(n_ch_convout)  )
        self.spec = ReshapeLayer(16 * n_ch_convout , 1)  
    def forward(self, x):
        x = self.conv0(x)
        x = self.conv1(x)
        x = self.conv2(x)
...
class Decoder_reshape(nn.Module):
    def __init__(self, n_ch_convout = 256, n_ch_out = 3):
        super().__init__()
        self.spec = ReshapeLayer(n_ch_convout, 16)
        self.tconv0 = nn.Sequential(
            nn.ConvTranspose2d(n_ch_convout, 128, kernel_size=(3,3), stride=1, padding=(1,1), output_padding=(0,0)), 
            nn.BatchNorm2d(128),
            nn.ReLU())
        self.tconv1 = nn.Sequential(
            nn.ConvTranspose2d(128, 128,          kernel_size=(3,3), stride=4, padding=(1,1), output_padding=(3,3)), #>>>>>>
            nn.BatchNorm2d(128),
            nn.ReLU())
        self.tconv2 = nn.Sequential(
            nn.ConvTranspose2d(128, 64,           kernel_size=(3,3), stride=2, padding=(1,1), output_padding=(1,1)), 
            ...
            nn.ReLU())
        self.tconv3 = nn.Sequential(
            nn.ConvTranspose2d(64, 32,            kernel_size=(3,3), stride=1, padding=(1,1), output_padding=(0,0)), 
            nn.BatchNorm2d(32),
            nn.ReLU())
